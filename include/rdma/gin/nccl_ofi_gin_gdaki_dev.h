...
struct nccl_ofi_gin_gdaki_dev_handle {
	 ...
	 *
	 * `net.signal(team, peer, ...)` (used by ncclBarrierSession) routes through
	 * ncclGinApi_Put with hasWins=false, bytes=0. EFA needs an actual remote
	 * memory destination to bump the receiver's FI_REMOTE_WRITE counter, so
	 * the plugin allocates a small buffer per createContext, registers it on
	 * the proxy domain, and allgathers the (local_addr, rkey) per rank. The
	 * GPU kernel uses these to post a 4-byte RDMA write to the peer's scratch
	 * region whenever it needs a signal-only delivery.
	 */
