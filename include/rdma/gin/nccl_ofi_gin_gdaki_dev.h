 ...
 * NCCL_OFI_GDAKI_PEER_WINDOW is 2^PSEQ_BITS: it is both the maximum number of
 * writes outstanding to one peer (the cap put enforces) and the width in bits of
 * that peer's completion bitmask, so pseq wraps at PEER_WINDOW without aliasing a
 * live write. NCCL_OFI_GDAKI_PEER_BITS_WORDS is PEER_WINDOW expressed in 64-bit
 * words, which is that bitmask's storage size.
 *
...
#define NCCL_OFI_GDAKI_PSEQ_BITS 8u
#define NCCL_OFI_GDAKI_PEER_WINDOW (1u << NCCL_OFI_GDAKI_PSEQ_BITS)
#define NCCL_OFI_GDAKI_PEER_BITS_WORDS (NCCL_OFI_GDAKI_PEER_WINDOW / 64u)

/* req_id is 16 bits, so the bits pseq does not take belong to the peer id, and that
 * width is what caps the ranks this backend can address: a peer at or above
 * NCCL_OFI_GDAKI_MAX_PEERS would be truncated by the device's peer mask and its
 * completions would be attributed to a different peer. createContext rejects such a
 * communicator rather than letting put alias silently. */
#define NCCL_OFI_GDAKI_MAX_PEERS (1u << (16u - NCCL_OFI_GDAKI_PSEQ_BITS))
...
struct nccl_ofi_gin_gdaki_dev_endpoint_handle {
	 ...
	 * EFA_CNTR_MASK). The SQ-overflow backpressure spin and the blocking
	 * peer-less Flush read it. */
...
/**
 * Per-peer completion state, one entry per (context, peer), published by the host
 * CQ progress pass and read by FlushAsync and Wait.
 *
 * ordered_completed_count_per_peer is the contiguous prefix of that peer's posting
 * sequence: N means the peer's first N writes on this context have all completed
 * with no gaps.
 *
 * error_at is 0 when no failure is seen, otherwise the failed pseq plus 1 (the +1
 * keeps 0 meaning "none").
 *
 * Layout is shared with the NCCL mirror in
 * nccl_device/gin/efa_gda/gin_efa_gda_dev.h — keep them in sync.
 */
...
struct nccl_ofi_gin_gdaki_dev_handle {
	 ...
	 *
	 * peer_completion[p] (host-published, [nranks]) holds that peer's contiguous
	 * ordered_completed_count_per_peer plus its error position. Wait compares its
	 * snapshot ticket against this one word, so a single number per request
	 * suffices. */
	uint32_t *submitted_count_per_peer;
	volatile struct nccl_ofi_gin_gdaki_peer_completion *peer_completion;
