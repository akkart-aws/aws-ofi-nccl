...
struct nccl_ofi_gdaki_rail_t {
	...
	/*
	 * True when mode-zero fi_getinfo returned -FI_ENODATA and the rail had
	 * to be opened with FI_CONTEXT2. Such a rail supports backend version 1,
	 * whose CQ is device-polled, but not backend version 2 host polling.
	 */
	bool backend_v1_only = false;
};
