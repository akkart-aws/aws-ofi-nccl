...
public:
	 ...
	 *
	 * A wide endpoint asks for the 128B WQE; a narrow endpoint keeps the
	 * 64B entry and the full SQ depth.
	 */
	void open(struct fid_domain *domain, struct fi_info *ref_info,
		  struct fid_cq *cq, bool wide_wqe);
...
private:
	...
	/* Consumer cursor and expected phase for the ring walk. */
	uint64_t cc = 0;
	int phase = 1;
...
public:
	 ...
	 */
	void open(struct fid_domain *domain, struct fi_info *ref_info,
		  struct fi_efa_ops_gda *gda_ops, struct fid_cq *cq, uint64_t cntr_flags, bool wide_wqe);
