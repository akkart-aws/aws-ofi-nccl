						     ...
						     ncclNetDeviceHandle_v11_t **devHandle)
{
	...
	try {
		...
		for (int ctx_id = 0; ctx_id < nContexts; ctx_id++) {
			...
			ctx->data[ctx_id]->open(ofi_domain, gda_info, gda_ops, scq,
						FI_WRITE | FI_READ, /* wide_wqe */ false);
			if (local_n_sc > 0) {
			...
			ctx->pvdata[ctx_id]->open(ofi_domain, gda_info, gda_ops, scq, FI_WRITE, /* wide_wqe */ true);
