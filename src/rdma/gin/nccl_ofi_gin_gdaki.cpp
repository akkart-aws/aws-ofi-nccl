						     ...
						     ncclNetDeviceHandle_v11_t **devHandle)
{
	...
	try {
		...
		for (uint16_t r = 0; r < ctx->effective_rails; r++) {
			...
			const auto &gda_rail = ctx->gin_domain->get_rail(r);
			if (backend_version == NCCL_OFI_GDAKI_BACKEND_VERSION_2 &&
			    gda_rail.backend_v1_only) {
				throw std::runtime_error(
					"gin GDAKI: backendVersion 2 completion polling "
					"requires efa-direct without FI_CONTEXT2; "
					"libfabric predates efa-direct-without-FI_CONTEXT2 "
					"support");
			}
			...
			if (PlatformManager::get_global().get_platform().config_gdaki_domain(
				    dom_r, gda_rail.info.get()) != 0) {
				NCCL_OFI_WARN("gin GDAKI: not supported on this platform; "
					      "cannot create a GDAKI context here");
				return ncclInvalidUsage;
			}

			gda_ops_rail[r] = gda_rail.gda_ops;
			gda_info_rail[r] = gda_rail.info.get();
