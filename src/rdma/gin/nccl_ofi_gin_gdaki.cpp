...
/*
 * Shared stub body for the GDAKI plugin-table entry points when
 * libfabric was built without FI_EFA_GDA_OPS. In practice this path
 * is unreachable at runtime: nccl_ofi_gin_gdaki_enabled() returns
 * false on such a build, so nccl_ofi_gin_api.cpp never selects the
 * GDAKI plugin table in the first place. The stub exists so the
 * translation unit compiles against older libfabric headers.
 *
 * When HAVE_DECL_FI_EFA_GDA_OPS == 1 (the build that runs in
 * production), every !HAVE_DECL_FI_EFA_GDA_OPS branch below is
 * preprocessed out and no caller of this helper remains; the
 * [[maybe_unused]] attribute suppresses -Werror=unused-function in
 * that configuration.
 */
[[maybe_unused]] static ncclResult_t gdaki_not_supported(const char *op)
{
	NCCL_OFI_WARN("gin GDAKI: %s requires FI_EFA_GDA_OPS; plugin "
		      "was built without libfabric GDA ops support", op);
...
#if !HAVE_DECL_FI_EFA_GDA_OPS
	(void)ginCtx;
	return gdaki_not_supported("destroyContext");
#else
...
{
#if !HAVE_DECL_FI_EFA_GDA_OPS
	(void)collComm; (void)data; (void)size; (void)type; (void)mrFlags;
	(void)mhandle; (void)ginHandle;
	return gdaki_not_supported("regMrSym");
#else
...
{
#if !HAVE_DECL_FI_EFA_GDA_OPS
	(void)collComm; (void)mhandle;
	return gdaki_not_supported("deregMrSym");
#else
