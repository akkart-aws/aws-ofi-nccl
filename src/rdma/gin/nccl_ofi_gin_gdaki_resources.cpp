 ...
 *
 * Mirrors the proxy plugin's get_gin_hints pattern in
 * nccl_ofi_gin_resources.cpp: attributes the GDAKI endpoint requires
 * are hinted explicitly, and the fabric / domain names are copied from
 * the proxy's reference fi_info to narrow the fi_getinfo result to the
 * same efa-direct domain the proxy already uses.
 */
static void get_gdaki_hints(struct fi_info &hints, struct fi_info *ref_info)
{
	hints.caps = ref_info->caps;
	hints.mode = ref_info->mode;
	hints.addr_format = ref_info->addr_format;

	hints.ep_attr->type = ref_info->ep_attr->type;

	hints.domain_attr->mr_mode = ref_info->domain_attr->mr_mode;
	hints.domain_attr->cq_data_size = ref_info->domain_attr->cq_data_size;
			 ...
			 void *sq_buf_dev, void *sq_db_dev)
{
	...
	h.sq.wq.phase = 0;
	h.sq.max_inline_data = 32;
	h.sq.max_rdma_sges = 2;
