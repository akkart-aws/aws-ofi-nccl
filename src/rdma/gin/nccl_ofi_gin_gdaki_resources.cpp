			     ...
			     struct fid_cq *cq, bool wide_wqe)
{
	 ...
	 * fi_endpoint sizes the QP's inline capacity from info->tx_attr->inject_size
	 * and its depth from info->tx_attr->size, so both are set on the returned
	 * info rather than in the getinfo hints, which efa-direct rejects without
	 * FI_CONTEXT2. The SQ is a fixed byte budget, so a 128B entry holds half
	 * the entries of a 64B one. */
	if (wide_wqe) {
		info->tx_attr->inject_size = gdaki_wide_wqe_inject_size;
		info->tx_attr->size /= 2;
	}
