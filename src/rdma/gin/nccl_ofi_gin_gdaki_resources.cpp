...
/*
 * Inline-data capacity requested for a wide endpoint.
 *
...
static gdaki_efa_dp_context gdaki_create_efa_dp_context(int backend_version)
{
	...
	efa_cuda_host_context *ctx = efa_cuda_host_context_create(major, 0, 0);
	if (ctx == nullptr) {
		throw std::runtime_error(
 ...
 */
static struct fi_info *get_gdaki_info(struct fi_info *ref_info, bool wide_wqe)
{
	struct fi_info *hints = fi_allocinfo();
	if (hints == nullptr) {
		throw std::runtime_error("fi_allocinfo for GDAKI hints failed");
	}
	get_gdaki_hints(*hints, ref_info, wide_wqe);
			     ...
			     struct fid_cq *cq, bool wide_wqe)
{
	...
	info = get_gdaki_info(ref_info, wide_wqe);
...
void gdaki_endpoint::open(struct fid_domain *domain, struct fi_info *ref_info,
			  struct fid_cq *cq, bool wide_wqe)
{
	endpoint.open(domain, ref_info, cq, wide_wqe);
	endpoint.enable();
}
			       ...
			       uint64_t cntr_flags, bool wide_wqe)
{
	...
	base.endpoint.open(domain, ref_info, cq, wide_wqe);
			     ...
			     struct fi_efa_ops_gda *gda_ops, struct fid_cq *cq)
{
	...
	base.endpoint.open(domain, ref_info, cq, /* wide_wqe */ false);
...
uint32_t gdaki_completion_state::progress_cq(gdaki_host_cq &cq, int ctx_id, size_t max_iter)
{
	...
	for (size_t iter = 0; iter < max_iter; ++iter) {
		...
		/* The request id spans two fields of a TX completion: the low 16 bits in
		 * common.req_id and the upper 48 in req_id_ex, so a send-queue entry is
		 * read as an efa_io_tx_cdesc to recover the value the device stamped. */
		const struct efa_io_tx_cdesc *tx_cqe =
			reinterpret_cast<const struct efa_io_tx_cdesc *>(entry);
		const uint64_t req_id = (uint64_t)cqe->req_id |
					((uint64_t)tx_cqe->req_id_ex.w[0] << 16) |
					((uint64_t)tx_cqe->req_id_ex.w[1] << 32) |
					((uint64_t)tx_cqe->req_id_ex.w[2] << 48);
