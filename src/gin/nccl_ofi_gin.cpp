...
int nccl_ofi_gin_comm::iput_signal_deliver_all(uint32_t peer_rank)
{
	...
	while (true) {
		...
		/* If delivering this signal would overflow the ack_count field,
		   stop and send the ACK for what we have so far. The remaining
		   signals will be delivered on the next call. */
		if (any_ack_requested) {
			uint32_t count = ((next_seq_num - first_acked_seq_num + 1) & GIN_IMM_SEQ_MASK);
			if (count > 0x3FF)
				break;
		}
	...
	/* Send one range ACK covering all ack-requested signals delivered in
	   this call, instead of one ACK per signal. deliver_all walks the
	   contiguous deliverable range, so first_acked and highest_acked
	   bound the range. Seq_nums in between that did not request an ACK
	   are harmlessly skipped on the sender side (they never set
	   ack_outstanding). */
	if (any_ack_requested) {
		uint32_t count = ((highest_acked_seq_num - first_acked_seq_num + 1) & GIN_IMM_SEQ_MASK);
		ret = writedata_ack(*this, peer_rank, highest_acked_seq_num, count);
	}
